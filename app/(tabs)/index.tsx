  ...
  return (
    <View style={styles.group}>
      <View style={styles.groupHead}>
        ...
        <AppText style={styles.groupGame} numberOfLines={1}>{group.gameName}</AppText>
        <View style={[styles.countBadge, group.missed && styles.missedBadge]}>
          <AppText style={styles.countBadgeText}>{group.missed ? '놓친 소식' : '새 소식'} {group.items.length}</AppText>
        </View>
      </View>
      <FeedCard item={featured} density="home" />
  ...
  return (
    <View style={styles.eventCard}>
      ...
      <View style={styles.eventDates}>
        <Ionicons name="calendar-outline" size={14} color={theme.color.textMuted} />
        <AppText style={styles.eventDateText}>{formatDateOnly(item.startsAt)} ~ {formatDateOnly(item.endsAt)}</AppText>
      </View>
    </View>
  );
}
        ...
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh()} tintColor={theme.color.neonYellow} colors={[theme.color.neonYellow]} progressBackgroundColor={theme.color.surfaceContainerHigh} />}>
          ...
          <View style={{ paddingHorizontal: layout.margin }}>
            <View style={styles.sectionHead}>
              <View><AppText style={styles.sectionTitle}>마이픽</AppText><AppText style={styles.sectionDescription}>{selectedCount > 0 ? `${selectedCount}개 게임의 업데이트를 모았어요` : '내 게임을 고르면 맞춤 소식을 보여드려요'}</AppText></View>
              <AppText variant="data" style={styles.sectionStatus}>{loading ? '동기화' : homeTab === 'news' ? `${groups.length}개 게임` : `${events.length}개 진행`}</AppText>
            </View>
...
const styles = StyleSheet.create({
  ...
  sectionHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', borderBottomWidth: 1, borderBottomColor: theme.color.outlineVariant, paddingBottom: 12, marginBottom: 14, gap: 12 }, sectionTitle: { fontFamily: theme.font.headlineSemi, fontSize: 22, color: theme.color.onSurface }, sectionDescription: { fontFamily: theme.font.body, fontSize: 12, color: theme.color.textMuted, marginTop: 3 }, sectionStatus: { marginBottom: 3, color: theme.color.neonYellow },
  segmentWrap: { flexDirection: 'row', padding: 4, gap: 4, backgroundColor: theme.color.surfaceContainerHigh, borderWidth: 1, borderColor: theme.color.outlineVariant, borderRadius: theme.radius.sm, marginBottom: 16 }, segmentItem: { flex: 1, alignItems: 'center', paddingVertical: 11, borderRadius: 4 }, segmentItemActive: { backgroundColor: theme.color.primaryContainer }, segmentText: { fontFamily: theme.font.bodySemi, fontSize: 13, color: theme.color.textMuted }, segmentTextActive: { color: theme.color.onPrimary },
  group: { marginBottom: 16 }, groupHead: { flexDirection: 'row', alignItems: 'center', gap: 7, marginBottom: 8 }, gameMark: { width: 6, height: 6, borderRadius: 3, backgroundColor: theme.color.neonYellow }, groupGame: { flex: 1, minWidth: 0, fontFamily: theme.font.bodySemi, fontSize: 14, color: theme.color.onSurface }, countBadge: { paddingHorizontal: 7, paddingVertical: 4, borderRadius: 5, backgroundColor: 'rgba(202,255,0,0.14)' }, missedBadge: { backgroundColor: 'rgba(177,177,177,0.14)' }, countBadgeText: { fontFamily: theme.font.bodySemi, fontSize: 11, color: theme.color.neonYellow },
  ...
  eventCard: { marginBottom: 10, padding: 15, borderRadius: theme.radius.xl, backgroundColor: theme.color.surfaceContainer, borderWidth: 1, borderColor: theme.color.outlineVariant }, eventTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }, eventBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 5, backgroundColor: 'rgba(190,124,255,0.16)' }, eventBadgeText: { fontFamily: theme.font.bodySemi, fontSize: 11, color: theme.color.neonPurple }, eventCountdown: { fontFamily: theme.font.bodySemi, fontSize: 12, color: theme.color.neonYellow }, eventTitle: { fontFamily: theme.font.bodySemi, fontSize: 16, lineHeight: 22, color: theme.color.onSurface }, eventGame: { fontFamily: theme.font.body, fontSize: 12, color: theme.color.textMuted, marginTop: 5 }, eventDates: { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 12, paddingTop: 10, borderTopWidth: 1, borderTopColor: theme.color.outlineVariant }, eventDateText: { fontFamily: theme.font.body, fontSize: 12, color: theme.color.textMuted },
