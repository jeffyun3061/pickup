...
function groupByGame(items: ContentItem[], readIds: string[], missed: boolean): NewsGroup[] {
  ...
  for (const item of items) {
    const group = groups.get(item.gameId) ?? { gameId: item.gameId, gameName: item.gameName, items: [], missed };
  ...
  return [...groups.values()]
    .map((group) => ({
      ...
      items: [...group.items].sort((a, b) =>
        Number(readIds.includes(a.id)) - Number(readIds.includes(b.id))
...
function NewsGroupCard({ group }: { group: NewsGroup }) {
  ...
  return (
    <View style={styles.group}>
      <FeedCard item={featured} density="home" groupCount={group.items.length > 1 ? group.items.length : undefined} />
      {group.items.length > 1 ? (
        <View style={styles.moreItems}>
          {group.items.slice(1).map((item) => (
            <Pressable key={item.id} onPress={() => { readStore.add(item.id); router.push(`/content/${item.id}`); }} style={({ pressed }) => [styles.moreItem, readStore.has(item.id) && styles.moreItemRead, pressed && styles.moreItemPressed]} accessibilityRole="button" accessibilityLabel={`${item.gameName} ${item.title}${readStore.has(item.id) ? ' 읽음' : ''}`}>
...
function TodayNoNewsState({ onRefresh }: { onRefresh: () => void }) {
  return (
    <View style={styles.noNewsPanel}>
      <Image source={imageAssets.mascot} style={styles.noNewsMascot} resizeMode="cover" />
      <AppText variant="subtitle" style={styles.noNewsTitle}>오늘은 새로운 소식이 없어요</AppText>
      <AppText variant="caption" style={styles.noNewsDescription}>피키가 계속 확인하고 있어요. 소식이 올라오면 바로 알려드릴게요.</AppText>
      <Pressable onPress={onRefresh} style={({ pressed }) => [styles.noNewsRefresh, pressed && styles.noNewsRefreshPressed]} accessibilityRole="button">
        <Ionicons name="refresh" size={15} color={theme.color.onPrimary} />
        <AppText style={styles.noNewsRefreshText}>새로고침</AppText>
      </Pressable>
    </View>
  );
}
        ...
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh()} tintColor={theme.color.neonYellow} colors={[theme.color.neonYellow]} progressBackgroundColor={theme.color.surfaceContainerHigh} />}>
          ...
          <View style={{ paddingHorizontal: layout.margin }}>
            ...
            {!loading && homeTab === 'news' ? <>
              ...
              {!hasHomeNews && selectedCount > 0 ? <TodayNoNewsState onRefresh={() => void refresh()} /> : null}
              {!hasHomeNews && selectedCount === 0 ? <EmptyState icon="game-controller-outline" title="내 게임을 등록해보세요" description="고른 게임의 오늘 소식을 모아드릴게요." actionLabel="내 게임 고르기" onAction={() => router.push('/(tabs)/games')} /> : null}
            </> : null}
