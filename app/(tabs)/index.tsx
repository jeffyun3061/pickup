  ...
  return (
    <View style={styles.group}>
      <View style={styles.groupHead}>
        ...
        <View style={[styles.countBadge, group.missed && styles.missedBadge]} accessibilityLabel={`${group.items.length}건`}>
          <AppText style={styles.countBadgeText}>{group.items.length}</AppText>
        </View>
      </View>
        ...
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh()} tintColor={theme.color.neonYellow} colors={[theme.color.neonYellow]} progressBackgroundColor={theme.color.surfaceContainerHigh} />}>
          ...
          <View style={{ paddingHorizontal: layout.margin }}>
            <View style={styles.sectionHead}>
              ...
              <AppText variant="data" style={styles.sectionStatus}>{loading ? '동기화' : homeTab === 'news' ? `${selectedCount}개 게임` : `${events.length}개 진행`}</AppText>
            </View>
...
const styles = StyleSheet.create({
  ...
  sectionHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: theme.color.outlineVariant, paddingBottom: 9, marginBottom: 10, gap: 12 }, sectionTitle: { fontFamily: theme.font.headlineSemi, fontSize: 22, color: theme.color.onSurface }, sectionStatus: { color: theme.color.neonYellow },
  ...
  group: { marginBottom: 12 }, groupHead: { flexDirection: 'row', alignItems: 'center', gap: 7, marginBottom: 6 }, gameMark: { width: 6, height: 6, borderRadius: 3, backgroundColor: theme.color.neonYellow }, groupGame: { flex: 1, minWidth: 0, fontFamily: theme.font.bodySemi, fontSize: 14, color: theme.color.onSurface }, countBadge: { minWidth: 16, alignItems: 'flex-end' }, missedBadge: { opacity: 0.65 }, countBadgeText: { fontFamily: theme.font.bodySemi, fontSize: 13, color: theme.color.neonYellow },
